...
void run()     // ahead
{
  ...
  analogWrite(Right_motor_en,180);//PWM--Pulse Width Modulation(0~255). right motor go speed is 255.
   digitalWrite(Left_motor_go,HIGH);  // set left motor go ahead
  digitalWrite(Left_motor_back,LOW);
  analogWrite(Left_motor_en,220);//PWM--Pulse Width Modulation(0~255).left motor go speed is 135.
...
void left()        //turn left
{
  digitalWrite(Right_motor_go,HIGH);	// right motor go ahead
  digitalWrite(Right_motor_back,LOW);
  analogWrite(Right_motor_go,150); // PWM--Pulse Width Modulation(0~255) control speed，right motor go speed is 255.
...
void spin_left(int time)   //Left rotation
{
  digitalWrite(Right_motor_go,HIGH);// right motor go ahead
  digitalWrite(Right_motor_back,LOW);
  analogWrite(Right_motor_go,150);// PWM--Pulse Width Modulation(0~255) control speed ,right motor go speed is 200.
  analogWrite(Right_motor_back,0);
  digitalWrite(Left_motor_go,LOW);   // left motor back off
  digitalWrite(Left_motor_back,HIGH);
  analogWrite(Left_motor_go,0); 
  analogWrite(Left_motor_back,150);// PWM--Pulse Width Modulation(0~255) control speed,left motor back speed is 200.
  delay(time * 10);
}
...
void right()      //turn right
{
  ...
  digitalWrite(Left_motor_back,LOW);
  analogWrite(Left_motor_go,150);// PWM--Pulse Width Modulation(0~255) control speed ,left motor go speed is 255.
  analogWrite(Left_motor_back,0);
  //delay(time * 100);
}
...
void spin_right(int time)   //Right rotation
{
  ...
  digitalWrite(Left_motor_go,HIGH);// left motor go ahead
  digitalWrite(Left_motor_back,LOW);
  analogWrite(Left_motor_go,150);// PWM--Pulse Width Modulation(0~255) control speed 
  analogWrite(Left_motor_back,0);
  delay(time * 10);
}
...
void back(int time)   //back off 
{
  ...
  analogWrite(Right_motor_en,200);// PWM--Pulse Width Modulation(0~255) control speed
  digitalWrite(Left_motor_go,LOW);  //left motor back off
  digitalWrite(Left_motor_back,HIGH);
  analogWrite(Left_motor_en,200);// PWM--Pulse Width Modulation(0~255) control speed
